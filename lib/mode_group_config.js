 ...
 *
 * Class SRP: Store and validate the configuration for a single set of run mode-specific files
 */
...
ModeGroupConfig.prototype.get_dest_filepath = function(mode) {
  return path.join(this.dest, this.get_src_filename(mode).replace(mode + ".", ""));
};

ModeGroupConfig.prototype.get_dest_glob = function() {
  return path.join(this.dest, path.basename(this.get_src_glob()));
};

ModeGroupConfig.prototype.get_src_filepath_at_dest = function(mode) {
  return path.join(this.dest, path.basename(this.get_src_glob().replace("*", mode)));
}
