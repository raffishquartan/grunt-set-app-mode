 ...
 *
 * Class SRP: Register GSAM with Grunt as a plugin
 */
...
GruntSetAppMode.prototype.run = function() {
  ...
  var mode_groups = new ModeGroupConfigCollection(this.files, config.get_expected_modes());
  var exec = new SetAppModeExecutor({ config: config, mode_groups: mode_groups});
  exec.apply_all();
};
